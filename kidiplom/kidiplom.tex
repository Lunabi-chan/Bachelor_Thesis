...
Entity Framework Core (EF Core) je oficiální, open-source a cross-platform Object-Relational Mapper (ORM). Slouží jako moderní nástroj pro přístup k datům, který mapuje \csharp{} objekty na relační databáze a eliminuje potřebu psát většinu ručního kódu pro komunikaci s databází.

\subsubsubsection{AutoMapper}
AutoMapper je open-source knihovna pro \csharp{} a .NET, která slouží jako objektový mapper k automatizaci převodu dat mezi různými typy objektů. Eliminuje nutnost psát redundantní kód pro ruční kopírování vlastností, například při mapování entit z databáze na DTO (Data Transfer Objects) nebo ViewModels.
...
Česky \uv{Čistá architektura} je návrhový vzor, který definuje strukturu softwarového systému tak, aby byla nezávislá na frameworku, UI, databázi a externích agentech. Díky oddělení byznysové logiky a technických detailů, zajišťuje snadnou údržbu, testovatelnost a abstrakci mezi vrstvami.
...
\begin{itemize}
\item \textbf{/AssignmentEvaluation} slouží k parsování poskytnutého URL ke Github repozitáři, zjistí se programovací jazyk pomocí GitHub API
\item \textbf{/Services} je popis poskytovaných služeb.
\begin{itemize}
\item \textit{Assignment}: spravuje životní cyklus assignmentu a stav ukládá do databáze.
\item \textit{Result}: zpracovává výstup po vyhodnocení, zparsuje stdout na build/run výstup, nastavuje příznaky úspěchu a ukládá výsledek do databáze.
\item \textit{Value}: kontroluje hodnoty výstupu. Pro každé zadání by se měl tento modul měnit.
\end{itemize}
\item \textbf{/Validator} je složka pro služby, které se týkají validace.
\end{itemize}
...
\begin{itemize}
\item \textbf{installPrerequisities.sh}: tento skript se nevyužívá za běhu kontejneru, ale zajišťuje instalaci dodatečných systémových nástrojů při vytváření obrazu. Stahuje git, jq pro manipulaci s JSON datami, curl pro odeslání dat a dos2unix pro konverzi řádkových konců mezi formáty specifické pro UNIX a Windows.

\item \textbf{checkVariables.sh}: skript, který kontroluje přítomnost potřebných proměnných prostředí (environment variables) při startu kontejneru. Prvně jestli byl přiložen URL ke GitHub repozitáři a poté jestli má kontejner přístup k cestě pro access token. Ten je nezbytný jestli je repozitář soukromý. Ale i kdyby byl veřejný, je potřeba nastavit access token s podman secrets.

\item \textbf{generateValidationResult.sh} a \textbf{reportResult.sh}: tyto skripty se starají o vygenerování a zformátování DTO s informací ohledně průběhu projektu uchazeče a jeho následného odeslání pomocí služby RabbitMQ do zprávové fronty. 

\item \textbf{validate.sh}: obsahuje logiku pro stahování GitHub repozitáře, vyjmutí informací, spuštění konkrétní implementace jazykového skriptu \textit{start.sh} s timeoutem a vypsání následných zpráv po ukončení běhu projektu.

\item \textbf{run.sh}: hlavní řídící skript, který orchestruje a spouští ostatní skripty. 
\end{itemize}

Konkrétní implementace pro jednotlivé jazykové se nachází pod složkami s jejich zkratkou. Využívá se výše popsaný základ se svou vlastní logikou a balí je do výsledného kontejnerového obrazu. Každá složka obsahuje: 

\begin{itemize}
\item \textbf{runnerScript-<language\_extension>.sh}: tento skript reprezentuje konkrétní implementaci daného jazyka. Kontroluje přítomnost povinných souborů v projektu a volá specifické příkazy technologie ke kompilaci a spuštění běhu.

\item \textbf{Dockerfile}: definuje předpis pro sestavení obrazu. Přibalí se zde sdílené skripty, stáhnou se dodatečné systémové příkazy dle \textit{../installPrerequisities.sh}, přejmenuje se skript daného jazyka na \textit{start.sh} a nastaví se ENTRYPOINT na skript \textit{run.sh} 
\end{itemize}

Díky této architektuře je proces přidání nového jazyka velice snadné. Detailní popis je v dokumentaci nacházející se v \textit{documentation/testRunnerDocumentation.md}. 
