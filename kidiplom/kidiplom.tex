...
Backend je implementován v programovacím jazyce \csharp{} na platformě .NET8. .NET je bezplatná multiplatformní opensourcová vývojářská platforma pro vytváření mnoha druhů aplikací od společnosti Microsoft. 

\subsubsection{ASP.NET Core Web API}
ASP.NET Core Web API je framework určený pro vývoj RESTových webových služeb na platformě .NET. Umožňuje vytvářet HTTP rozhraní, prostřednictvím kterého mohou mezi sebou komunikovat jednotlivé aplikace nezávisle na použité technologii.
...
Podman (s alternativní možností Dockeru) je open-source nástroj pro správu kontejnerů kompatibilní se standardem OCI (\textit{Open Container Initiative}). Poskytuje téměř totožné příkazové rozhraní jako Docker, díky čemuž lze většinu existujících \texttt{Dockerfile} i příkazů používat beze změny.
...
Pro bezpečné předávání citlivých informací využívá Podman funkci \textit{Podman Secrets}. To umožňuje ukládat tajné hodnoty, například přístupové tokeny nebo hesla, mimo samotný kontejnerový obraz. Secret je při spuštění kontejneru připojen jako dočasný soubor, který je dostupný pouze běžícímu kontejneru a není součástí výsledného obrazu ani proměnných prostředí. Tím je minimalizováno riziko neúmyslného zveřejnění citlivých údajů.

Další výhodou tohoto nástroje je řízení kontejnerů přes jeho REST API. Prostřednictvím HTTP rozhraní modul \texttt{PodmanManager} vytváří nové kontejnery, předává jim vstupní proměnné, sleduje jejich stav a po dokončení práce zajišťuje jejich smazání.
...
\subsubsection{BASH a POSIX skriptování}
BASH (Bourne Again Shell) je příkazový interpret a skriptovací jazyk běžný v prostředí Linuxu. Ve~spojení se standardními nástroji pro zpracování textu (jako \kiinlinecode{text}{!}{sed} nebo \kiinlinecode{text}{!}{xmlstarlet}) umožňuje psát skripty pro automatizaci systémových kroků, manipulaci se soubory nebo úpravu textových dat přímo v přikazovém řádku.

\subsection{GitHub API}
GitHub API představuje RESTové rozhraní umožňující přístup k informacím uloženým na platformě GitHub. Přes HTTP požadavků lze získávat metadata o repozitářích, uživatelích, větvích nebo jednotlivých souborech.
...
Infrastrukturní vrstva zajišťuje komunikaci aplikace s~externími komponentami a databází. Mezi její hlavní součástí patří \textit{Entity Framework Core} pro~správu databáze. Dále se zde nachází \textit{PodmanManager}, který komunikuje se~službou Podman přes~HTTP~REST rozhraní. Ten řídí spouštění a mazání kontejnerů. Infrastruktura obsahuje napojení na message broker \textit{RabbitMQ}, jenž zajišťuje asynchronní příjem zpráv z kontejnerů a jejich předání na zpracování. A nakonec \textit{GitHub~API} pro zjištění jestli GitHub repozitář existuje a pro získání jeho programátorského jazyka.
...
\begin{itemize}
	...
	\item \textbf{\texttt{Dockerfile}} definuje předpis pro sestavení obrazu. Přibalí se zde sdílené skripty, stáhnou se dodatečné systémové příkazy podle skriptu \newline
	\texttt{../installPrerequisities.sh}, přejmenuje se skript daného jazyka na \texttt{start.sh} a nastaví se \kiinlinecode{text}{!}{ENTRYPOINT} na skript \texttt{run.sh}. 
\end{itemize}

Díky této architektuře je proces přidání nového jazyka velice snadný. V~dokumentaci \texttt{testRunnerDocumentation.md}, nacházející se v podsložce \newline 
\texttt{./documentations}, je detailní popis jak vytvořit takovou složku. 
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{Hromadné zpracování požadavků (\texttt{POST /api/evaluations/batch}):} Umožnilo by odeslat pole odkazů na repozitáře v jediném HTTP požadovaném rozhraní. Systém by následně hromadně vytvořil frontu úloh pro spuštění v runneru, což by zjednodušilo obsluhu pro větší počet úloh naráz.
		\item \textbf{Správa a promazávání dat (\texttt{DELETE /api/evaluations/all}):} Administrátorský endpoint určený k hromadnému mazání historických záznamů a výsledků (případně s možností filtrů podle data či stavu). To by usnadnilo údržbu databáze a uvolňování místa na disku.
	\end{itemize}
\end{itemize}
