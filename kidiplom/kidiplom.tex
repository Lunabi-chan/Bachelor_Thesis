...
\thanks{Ráda bych poděkovala svému vedoucímu práce Mgr. Petru Osičkovi, Ph.D., za trpělivost a vstřícnost při tvorbě práce. Dále děkuji Mgr. Markétě Trnečkové, Ph.D., za její cenné rady a čas. Děkuji také svým přátelům za podporu během studia i mimo něj. Nakonec děkuji firmě Edhouse, pod kterou práce vznikla.}
...
Hlavní motivací pro vznik platformy \textbf{InQuest} je proto ucelená automatizace prvotní fáze vyhodnocování úkolů. Systém standardizuje proces tím, že vyžaduje sjednocenou adresářovou strukturu odevzdávaných úkolů. Dále zajišťuje automatizované stažení kódu, jeho verifikaci a spuštění v~izolovaném kontejnerovém prostředí. Díky této automatizaci jsou mentoři odproštěni od repetitivních úkonů a mohou se soustředit na architekturu řešení, čistotu kódu a celkovou kvalitu návrhu.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|c|}
		...
		\textbf{Izolace} & Uzavřený Cloud & Sandbox & sandboxové prostředí \\ \hline
		\textbf{Kontrola adresáře} & Ne & Ne & Ne \\ \hline
		\textbf{Zadání na míru} & Omezená & Ano & Ano  \\ \hline
...
Vzhledem k~malému počtu entit bylo možné uchovávat záznamy přímo v~kolekcích v~paměti aplikace. EF Core ale poskytl jednotný způsob definice vztahů mezi entitami, dotazování pomocí LINQ a možnost pozdějšího nahrazení \textit{InMemory} poskytovatele trvalou relační databází.
...
\begin{figure}[h]
	...
	\caption{Čistá architektura, převzato z~\cite{clearch}}
	\label{fig:clearch}
\end{figure}
...
\begin{itemize}
	...
	\item \textbf{Prezentační} (\textit{Presentation}): vystavuje koncové body pro komunikaci přes HTTP. Přijímá požadavky klienta, předává je aplikační vrstvě a převádí výsledky zpracování do odpovídajících HTTP odpovědí.
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{Result}: zpracovává výstup po vyhodnocení, rozděluje standardní výstup na informace o~sestavení a spuštění, nastavuje příznaky úspěchu a ukládá výsledek do databáze.
...
\begin{itemize}
	...
	\item \textbf{\texttt{Dockerfile}} definuje předpis pro sestavení kontejnerového obrazu runneru. Přibalí se zde sdílené skripty a nainstalují se dodatečné systémové příkazy podle skriptu \texttt{../installPrerequisities.sh}. Jazykově specifický skript je při sestavení přejmenován na \texttt{start.sh} a jako vstupní bod kontejneru je nastaven skript \texttt{run.sh}.
\end{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item Dynamicky sestaví tag obrazu získaného z~kroku 3 a spustí \kiinlinecode{text}{!}{docker build}.
...
\subsection{Ukázka rozhraní Swagger UI}
...
Na obrázku~\ref{fig:swaggerOverview} je zobrazen přehled operací poskytovaných kontrolerem \textit{Assignments}. Rozhraní zpřístupňuje operace pro vytvoření požadavku na vyhodnocení, získání uložených zadání, opakované spuštění vyhodnocení a odstranění konkrétního záznamu. Jednotlivé operace lze ve Swagger UI rozbalit a následně přímo volat.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{graphics/SwaggerHome.png}
	\caption{Přehled endpointů systému ve Swagger UI}
	\label{fig:swaggerOverview}
\end{figure}

Příklad vytvoření požadavku na vyhodnocení je znázorněn na obrázku~\ref{fig:swaggerPost}. Endpoint \kiinlinecode{text}{!}{POST /api/Assignments} přijímá tělo ve formátu JSON obsahující URL GitHub repozitáře. Po stisknutí tlačítka \textit{Execute} odešle Swagger UI požadavek backendové aplikaci.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{graphics/SwaggerPOST.png}
	\caption{Odeslání požadavku na vyhodnocení ve Swagger UI}
	\label{fig:swaggerPost}
\end{figure}	

Při úspěšném přijetí požadavku vrací API stavový kód \kiinlinecode{text}{!}{201 Created} a objekt typu \texttt{AssignmentDto}, jak je znázorněno na obrázku~\ref{fig:swaggerPostResponse}. Odpověď obsahuje identifikátor vytvořeného zadání, URL repozitáře, čas vytvoření, aktuální stav a případný poslední výsledek.

V~uvedeném příkladu odpovídá hodnota položky \kiinlinecode{text}{!}{state} probíhajícímu vyhodnocení a položka \kiinlinecode{text}{!}{latestResult} zatím neobsahuje žádnou hodnotu. Důvodem je zpracování repozitáře, které v~okamžiku vytvoření odpovědi ještě nebylo dokončeno.

\begin{figure}[H]	
	\centering
	\includegraphics[width=0.9\textwidth]{graphics/SwaggerPOSTResponse.png}
	\caption{Odpověď API ve Swagger UI}
	\label{fig:swaggerPostResponse}
\end{figure}

Aktuální stav zadání a jeho výsledek lze následně získat prostřednictvím požadavku \kiinlinecode{text}{!}{GET /api/Assignments}. Ukázka na obrázku~\ref{fig:swaggerGet} zachycuje odpověď se stavovým kódem \kiinlinecode{text}{!}{200 OK} po dokončení vyhodnocení. Položka \kiinlinecode{text}{!}{latestResult} obsahuje identifikátor zpracovaného commitu, rozpoznaný programovací jazyk, výsledky inicializace, sestavení a spuštění projektu a související textové výstupy. Klientská aplikace tak může pomocí REST~API zjistit nejen aktuální stav zadání, ale také podrobnosti o~průběhu jeho zpracování.

\begin{figure}[H]
	\centering
	 \includegraphics[width=0.9\textwidth]{graphics/SwaggerGET.png}
	\caption{Získání dokončeného výsledku vyhodnocení pomocí Swagger UI}
	\label{fig:swaggerGet}
\end{figure}
...
\item[\texttt{text/}] \hfill \\
  ...
  archivu), tj.~zdrojový text práce a příloh, vložené obrázky apod.
